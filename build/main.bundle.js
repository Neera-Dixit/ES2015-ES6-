...
/***/ function(module, exports, __webpack_require__) {
	...
	var API = {
	        fetch: function fetch(param) {
	
	                return new Promise(function (resolve, reject) {
	
	                        $.ajax({
	                                url: _constants.jsonServerURL + param,
	                                type: 'GET',
...
/***/ function(module, exports) {
	...
	exports.jsonServerURL = jsonServerURL;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _xssFilters = __webpack_require__(5);
	...
	var ui = {
		renderPosts: function renderPosts(posts) {
			...
			var postData = posts.map(function (post) {
			...
			var postSelec = document.querySelector("#posts");
		...
		renderUsers: function renderUsers(users) {
			...
			var userSelec = document.querySelector("#users");
	...
	function commentTemplate(title, time) {
		...
		var safeTime = _xssFilters2.default.inHTMLData(time);
		return "<div>\n\t\t\t\t\t<div>Post : " + safeTitle + "</div>\n\t\t\t\t\t<div>" + safeTime + "</div>\n\t\t\t\t\t</div><br/>";
	}
	
	function userTemplate(user, image) {
		var safeUser = _xssFilters2.default.inHTMLData(user);
		var safeImg = _xssFilters2.default.inHTMLData(image);
		return "<div>\n\t\t\t\t\t<div>" + safeUser + "</div>\n\t\t\t\t\t</div><br/>";
	}
